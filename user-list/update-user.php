...
$original_user_id = $original_username = $original_user_role = '';
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    $original_user_id = clean_input($_POST['original-user-id']);
    $original_username = clean_input($_POST['original-username']);
    $original_user_role = clean_input($_POST['original-user-role']);
    ...
    }else if($userObj->userIdExistList($user_id, $original_user_id)){
        $user_id_valid = false;
    ...
    if ($userObj->checkExistingUser($user_id, $username, $original_user_id, $original_username)) {
    ...
    if($determiner == 'false'){
        if ($userObj->updateUser($original_user_id)) {
            echo json_encode(['status' => 'success']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Something went wrong when updaing user details.']);
        }
        ...
        if ($userObj->updateUser($original_user_id) && $userObj->updateAccount($original_user_id)) {
            echo json_encode(['status' => 'success']);
        }else{
